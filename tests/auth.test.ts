...
describe("Register Tests", () => {
  ...
  test("Middleware", async () => {
    ...
    const res2 = await request(app).get("/auth").set("Authorization", "Bearer " + user.accessToken).send();
    expect(res2.statusCode).toEqual(200);

    await request(app).post("/post").set("Authorization", "Bearer " + user.accessToken).send({
      "title": "Biker Post Title",
      "message": "bike post content",
      "owner": user.email
    });
    expect(res2.statusCode).toEqual(200);
  });
  ...
  test("Refresh Token", async () => {
    ...
    const res = await request(app).get("/auth").set("Authorization", "Bearer " + user.accessToken).send();
    expect(res.statusCode).not.toEqual(200);
    ...
    const res3 = await request(app).get("/auth").set("Authorization", "Bearer " + user.accessToken).send();
